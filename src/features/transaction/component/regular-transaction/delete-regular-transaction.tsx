...
export const DeleteRegularTransaction: FC<{ id: string }> = ({ id }) => {
  const mutationKey = `delete-${queryKeys.transaction.regular}-${id}`;
  const queryClient = useQueryClient();

  const { open, onOpenChange } = usePopupState();
  const { mutate } = useMutation({ mutationKey: [mutationKey], mutationFn: deleteRegularTransactionApi });
